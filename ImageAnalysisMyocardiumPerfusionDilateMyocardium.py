#This file will dilate the myocardium according to
#pre-specific % value
...
class ImageAnalysisDilateMyocardium():
	...
	def main(self):
		...
		print ("--- Minimize the distance between Centerline and Myocardium Surface")
		self.Volume_Registered=self.MinimizeDistance(Volume,Centerline_Coords)
	...
	#Minimize the distance between Myocardial Surface and Centerlines
	def Minimize_Distance(self,Volume,Centerlines):
		#Get the surface of the myocardium
		Surface=vtk.vtkDataSetSurfaceFilter()
		Surface.SetInputData(Volume)
		Surface.Update()
		
		#Get the array of coordinates of the Surface
		N_surface_pts=Surface.GetNumberOfPoints()
		#Compute the distance from the Centerlines to surface points
		Surface_coords=np.zeros(shape=(N_surface_pts,3))
		Dist_Surface_to_CL=np.zeros(N_surface_pts)
		for i in range(0,N_surface_pts):
