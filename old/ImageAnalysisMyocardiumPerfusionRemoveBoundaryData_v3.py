...
class ImageAnalysisMyocardiumRemoveBoundaryData():
	...
	def main(self):
		...
		if self.Args.Method=="Whole": #Scrap-off from whole myocardium
			...
			self.ProximityToSurface(Volume,SurfaceCoords,ThresholdArray,0)

		elif self.Args.Method=="Territories": #Scrap-off from each territory
			#Get the number of territories
			N_points=Volume.GetPointData().GetArray("TerritoryMaps").GetNumberOfValues()

			#Get all of the territory ids into a dictionary
			TerritoryValues={}
			max_territoryid=-10
			for i in range(0,N_points):
				value_=Volume.GetPointData().GetArray("TerritoryMaps").GetValue(i)
				if value_ not in TerritoryValues.keys(): TerritoryValues[value_]=[]
				else: TerritoryValues[value_].append(i)
				if value_>max_territoryid: max_territoryid=value_
			print (TerritoryValues.keys())

			print ("--- Scrap off N=%d layers from the Volume"%self.Args.Layers)
			#Loop over each territory
			for key in TerritoryValues.keys():
				print ("--- Looping over Territory: %d"%(key))
				
				#Now Find the distance from the volumetric node to the surface coordinate
				ThresholdArray=np.array([None]*Volume.GetNumberOfPoints())
				np.put(ThresholdArray,TerritoryValues[key],[False]*len(TerritoryValues[key]))
				
				#Get the surface nodes of the threshold
				Threshold_=vtk.vtkThreshold()
				Threshold_.SetInputData(Volume)
				Threshold_.ThresholdBetween(key,key)
				Threshold_.SetInputArrayToProcess(0, 0, 0,"vtkDataObject::FIELD_ASSOCIATION_POINTS","TerritoryMaps")
				Threshold_.Update()
				Threshold_=Threshold_.GetOutput()
				SurfaceCoords_=self.ExtractSurfaceCoordinates(Threshold_)
				self.ProximityToSurfaceMinDistance(Volume,SurfaceCoords_,ThresholdArray,0)
		else:
			print ("Method Argument is not valid")

	def ProximityToSurfaceMinDistance(self,Volume,SurfaceCoords, ThresholdArray,Counter):
		SurfaceCoords_T=np.transpose(SurfaceCoords)
		Npts=Volume.GetNumberOfPoints()
		for i in range(0,Npts):
			if ThresholdArray[i]!=None:
				coord_=Volume.GetPoint(i)
				dist_=DISTANCE.cdist([coord_],SurfaceCoords_T)
				print (SurfaceCoords[0])
				exit(1)
				dist_ =(coord_[0]-SurfaceCoords_T[0])**2
				dist_+=(coord_[1]-SurfaceCoords_T[1])**2
				dist_+=(coord_[2]-SurfaceCoords_T[2])**2
				dist_=dist_**0.5
				print (dist_)
				exit(1)

				print (len(dist_.min(axis=0)))
				exit(1)
				if dist_.min(axis=0)<0.2: ThresholdArray[i]=True
	...
	def ProximityToSurface(self,Volume,SurfaceCoords,ThresholdArray,Counter):
		...
		for i in range(Np_vol):
			if ThresholdArray[i] is None: continue
			else: #print the progress so far
		...
		self.WriteOutput(self.Args.OutputFileName+".vtu",Volume)	
		...
		##print ("--- Writing File: %s"%self.Args.OutputFileName+"_%s_layer_%d.vtu"%(self.Args.Method,Counter))
...
if __name__=="__main__":
        #Description
	parser = argparse.ArgumentParser(description="This script will scrap-off the MBF vales from the interior and exterior edges.")
	...
	#Method to shed off the layer (whole myocardium or individual territories)
	parser.add_argument('-Method', '--Method', type=str, required=False, default="Whole",dest="Method",help="The method to scrap off the myocardium layers: 'Whole' for the entire myocardium or 'Territories' for scraping off each territory")
