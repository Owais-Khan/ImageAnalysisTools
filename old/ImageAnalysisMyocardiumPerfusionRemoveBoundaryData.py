...
class ImageAnalysisMyocardiumRemoveBoundaryData():
	...
	def main(self):
		...
		if self.Args.Method=="Whole": #Scrap-off from whole myocardium
			...
			#Loop over each territory and get threshold values
			for i in range(0,max_territoryid-1):
				Threshold_=vtk.vtkThreshold()
				Threshold_.SetInputData(Volume)
				Threshold_.ThresholdBetween(i,i+1)
				...
				self.WriteOutput("Territory_%d.vtu"%i,Threshold_)
 
				#Now extract all of the surface mesh nodes fromt the volumetric data
				print ("--- Extracting the Surface Coordinates from the Volumetric Mesh")
				SurfaceCoords=self.ExtractSurfaceCoordinates(Threshold_)

				#Now Find the distance from the each of the volumetric node to the surface coordinate
				print ("--- Scrap off N=%d layers from the Volume"%self.Args.Layers)
				ThresholdArray=[False]*Threshold_.GetNumberOfPoints()
				self.ProximityToSurface(Threshold_,SurfaceCoords,ThresholdArray,0)
		else:
	...
	def ProximityToSurface(self,Volume,SurfaceCoords,ThresholdArray,Counter):
		if Counter==self.Args.Layers:
			...
			print (ThresholdArray[0],Counter)
