...
class ImageAnalysisMyocardiumPerfusionTerritoriesPlot():
	...
	def main(self):
		...
		Per80LV=np.percentile(MBF_Data_LV,80)
		...
		outfile.write("---80th Percentile: %.06f\n"%Per80LV)
		...
		for key_ in self.MBF_data.keys():
			if len(self.MBF_Labels[key_])>0:
				...
				outfile.write("---MeanNormalized75:  %.06f\n"%(np.mean(self.MBF_data[key_])/Per75LV))
				outfile.write("---StdevNormalized75: %.06f\n"%(np.std(self.MBF_data[key_])/Per75LV))
				outfile.write("---MeanNormalized80:  %.06f\n"%(np.mean(self.MBF_data[key_])/Per80LV))
				outfile.write("---StdevNormalized80: %.06f\n"%(np.std(self.MBF_data[key_])/Per80LV))
